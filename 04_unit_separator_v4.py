...
----- Version 4:
Displaying an error message do the user wont input more than one quantity.
"""
...
def unit_error(product_units, user_input):
    ...
    while not valid:
        ...
        print("Sorry, we are only able to calculate 'g', 'mg', 'l', "
              "and 'ml'")
        print(units)
        user_input = input("Please enter only the quantity of that brand. "
                           "(Don't enter more than one quantity, and use units"
                           " e.g. 'ml' or 'g'): ").lower()
